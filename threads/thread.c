...
bool cmp_wakeup (const struct list_elem *a_, const struct list_elem *b_, void *aux UNUSED);
void preempt(void);
...
void mlfqs_priority (struct thread *t) {
	if (t != idle_thread) {
		...
		// t->priority = sub_fp( sub_fp( int_to_fp(PRI_MAX), div_mixed( t->recent_cpu, 4 ) ), t->nice * 2 );		
...
void mlfqs_recent_cpu (struct thread *t) {
	if (t != idle_thread) {
		...
		t->recent_cpu = add_mixed( mult_fp( div_fp( mult_fp( int_to_fp(2), load_avg ) ,
		 add_mixed( mult_fp( int_to_fp(2), load_avg ), 1 ) ) , t->recent_cpu ), t->nice );
		// int decay = div_fp(mult_fp( int_to_fp(2), load_avg ), add_mixed( mult_fp( int_to_fp(2), load_avg ), 1 ) );
...
void mlfqs_recalc_recent_cpu (void) {
	...
	while (sleep_elem != list_end(&sleep_list)) {
		struct thread *now_sleep = list_entry(sleep_elem, struct thread, elem);

		mlfqs_recent_cpu(now_sleep);
		
		sleep_elem = list_next(sleep_elem);
	}
	
	while (ready_elem != list_end(&ready_list)) {
		struct thread *now_ready = list_entry(ready_elem, struct thread, elem);

		mlfqs_recent_cpu(now_ready);	
